...
<div class="container auto-max-width">
    ...
    <?php echo Form::open( array( 'url' => route('administration.taxonomy.units.create'), 'role' => 'form' ) ); ?>

        <div class="form-group<?php if( $errors->has('name') ){ ?> has-error<?php } ?>">
            <label for="name">Name</label>
            <input type="text" class="form-control" id="name" name="name" placeholder="Enter Taxonomy Unit name" value="{{ Input::old('name') }}" />
            @if ($errors->has('name') )
